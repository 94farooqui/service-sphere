    ...
    <div className='card bg-white p-2 rounded-md border'>
      <div>
        ...
        <form className='mt-4 text-sm flex flex-col gap-2'>
          ...
          <select onChange={(e)=>onValueChange(e)} defaultValue={bug.priority} className='input-select-style'>
            {priority.map(p => <option key={p.label} value={p.label}>{p.label}</option>)}
            </select>
          </div>
          <div className='grid grid-cols-[60px_auto]'>
          <label className='self-center'>Status</label>
          <select onChange={(e)=>onValueChange(e)} defaultValue={bug.status} className='input-select-style'>
          ...
          <select onChange={(e)=>onValueChange(e)} defaultValue={bug.type} className='input-select-style'>
